...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, title, metrics }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-xs p-4 select-none">
      <div 
        className="relative max-w-md w-full bg-[#0c0c0c] border border-border-subtle p-6 rounded-2xl shadow-2xl flex flex-col font-sans text-text-primary"
      ...
      >
        ...
        <div className="flex flex-col gap-4 w-full">
          {/* Link Copier Row */}
          <div className="flex items-center gap-2 w-full bg-surface-card border border-border-subtle p-2.5 rounded-xl">
            <input
              type="text"
              readOnly
              value={shareUrl}
              className="flex-1 bg-transparent text-xs text-text-secondary outline-none truncate font-mono"
            />
            <button
              onClick={handleCopyLink}
              className="px-3 py-1.5 rounded-lg bg-accent-purple text-white hover:bg-accent-violet transition text-xs font-semibold flex items-center gap-1.5 cursor-pointer shrink-0"
            >
          ...
          <div className="flex gap-2 w-full">
            {typeof navigator !== 'undefined' && typeof navigator.share === 'function' && (
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-surface-elevated border border-border-subtle text-white hover:border-text-secondary transition text-xs font-bold flex items-center justify-center gap-2 cursor-pointer"
              >
          ...
          {/* QR Code Segment */}
          <div className="flex flex-col items-center justify-center border border-border-subtle/50 bg-[#070707] p-4 rounded-2xl mt-2">
            <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider font-mono mb-3">Scan QR Code</span>
            <div className="bg-[#0c0c0c] p-2 border border-border-subtle rounded-xl flex items-center justify-center shadow-inner">
